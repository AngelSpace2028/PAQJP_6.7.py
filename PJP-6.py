...
class PJPCompressor:
    ...
    def algorithm_36_compress(self, data: bytes, time_limit: float = 300.0) -> bytes:
        """
        Algorithm 36:
        - Try pass values (powers of two, plus extra candidates for small files).
        ...
        candidates = set()
        # Powers of two from 1 to 2^23
        for i in range(24):
            candidates.add(1 << i)
        # For small files (≤40 bytes), add mean, median, and each chunk value
        if len(data) <= 40:
            # Mean and median
            mean = sum(values) // n
            candidates.add(mean)
            ...
            # Each chunk value itself (subtracting it makes that chunk zero)
            for v in values:
                candidates.add(v)
            # Also add a few random offsets around mean
        ...
        # Evaluate candidates, but ensure we don't exceed time limit
        for p in candidates:
        ...
        # Find the index (0-23) of the best pass (if not in powers, we fallback to nearest power)
        # Actually we need to store the pass itself in the header; we only have 1 byte for index (0-23)
        # So we must restrict the chosen pass to a power of two.
        # If the best candidate is not a power of two, we pick the power of two that gave the best metric
        # among the powers. But we already have those metrics; we can just keep track of the best power.
        # Simpler: we compute best among powers of two separately, and use that as the stored index.
        # For small files, we can also store the actual pass in extra bytes? But we want 1 byte overhead.
        # So we will just use the power-of-two index as the stored value.
        ...
        # So we re-evaluate powers of two and pick the best among them.
        best_idx = 0
        best_metric = float('inf')
    ...
    def compress_with_best_plus_block(self, infile: str, outfile: str,
        ...
        # 3. Algorithm 36
        print("\nRunning Algorithm 36 (24‑bit pass search with PJP)...")
        algo36_start = time.time()
        algo36_bytes = self.algorithm_36_compress(data, time_limit=300.0)
        algo36_time = time.time() - algo36_start
        algo36_size = len(algo36_bytes)
    ...
    # ------------------------------------------------------------------
    # Decompression (handles standard PJP, Zaden, and Algorithm 36)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Full self‑test (exhaustive) – now includes Zaden and Algorithm 36
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Test 2704 pairs & extraction check – now includes Zaden and Algorithm 36
    # ------------------------------------------------------------------
...
def main():
    ...
    while True:
        ...
        print("5) Full self‑test (now includes Zaden and Algorithm 36)")
        print("6) Decompress (extract)")
        print("7) Test 2704 pairs & extraction check (now includes Zaden and Algorithm 36)")
        ...
        if choice == 0:
            ...
            i = input("Input file: ").strip()
            o = input("Output file: ").strip() or i + ".pjp"
            # block_size=256, quantum_boost=False, time_limit=300 seconds
