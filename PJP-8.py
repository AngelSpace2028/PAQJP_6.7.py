...
- 65,535 transform pairs for Ultra compression (time‑limited to 300s)
...
- NEW: Transform 57 – 4‑byte XOR with most frequent pattern (adds many zeros)
- Ultra mode respects a 300‑second time limit (set via ULTRA_TIME_LIMIT)
"""
...
class UnifiedCompressor:
    ...
    # ------------------------------------------------------------------
    # Main compression methods (with time limit for ultra)
    # ------------------------------------------------------------------
    def compress_with_lzh(self, data: bytes, ultra: bool = True, time_limit: Optional[float] = None) -> bytes:
        ...
        def try_candidate(transform_header: bytes, transformed_data: bytes):
            ...
            # Verify correctness only if candidate is better
        ...
        # Single transforms (256) – all should be fast
        ...
        if ultra:
            # Pair transforms – potentially 65535, but we stop at time limit
        ...
        if best_bytes is None:
            # Fallback to raw + backend (which is always safe)
            fallback = self._encode_marker_raw() + self._compress_backend(data)
            if self._decompress_lzh_pipeline(fallback) != data:
    ...
    def compress_with_best(self, data: bytes, ultra: bool = True, time_limit: Optional[float] = None) -> bytes:
        if time_limit is None:
            time_limit = self.ULTRA_TIME_LIMIT
        start_time = time.time()
        ...
        # Helper to try a candidate, verify correctness only if it's better
        ...
        if best_bytes is None:
            # Fallback
            fallback = self._encode_marker_raw() + self._compress_backend(data)
            decomp, _ = self._decompress_auto(fallback)
            if decomp != data:
                raise RuntimeError("Fallback compression failed.")
            return fallback
        return best_bytes
    ...
    def compress_file(self, infile: str, outfile: str = "", ultra: bool = True, use_lzh: bool = False, time_limit: Optional[float] = None):
        try:
        ...
        try:
            if use_lzh:
                ...
                compressed = self.compress_with_best(data, ultra=ultra, time_limit=time_limit)
                default_suffix = ".pjp"
    ...
    def full_self_test(self) -> bool:
        ...
        try:
            compressed = self.compress_with_lzh(test_data, ultra=True, time_limit=30)  # limit to 30s for test
