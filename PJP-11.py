...
- NEW: 64/128/256/1024/2048-bit quantum-inspired XOR key transforms (option 9)
...
- ALL base transforms are individually lossless for every input.
...
# ---------- Prompt 2: Four optional backends ----------
...
class UnifiedCompressor:
    ...
    def set_quantum_qubits(self, q: int):
        if not USE_QUANTUM or not HAS_QISKIT:
            ...
            return
        if q < 1 or q > 2048:
            print("Qubit count must be between 1 and 2048.")
            return
        size = 1 << q if q <= 12 else 1024  # For large q we use block size 1024, no full state
        if q <= 12 and size > 1_000_000:
    ...
    def reverse_transform_02(self, d):
        if d == b'\x00':
            return b''
        if len(d) < 2:
            raise TransformError("Transform 02 reverse: data too short")
        pattern_index = d[0]
        t = bytearray(d[1:])
        pattern_values = self._get_pattern(4, pattern_index)
        for i in range(1, len(t), 4):
    ...
    def reverse_transform_03(self, d):
        if d == b'\x00':
            return b''
        if len(d) < 2:
            raise TransformError("Transform 03 reverse: data too short")
        rotation = d[0]
        t = bytearray(d[1:])
        for i in range(2, len(t), 5):
            if i < len(t):
    ...
    def reverse_transform_10(self, data: bytes) -> bytes:
        if len(data) < 1: raise TransformError("Transform 10 reverse: data too short")
        n = data[0]
        t = bytearray(data[1:])
        for i in range(len(t)): t[i] ^= n
        return bytes(t)
    ...
    def reverse_transform_13(self, d):
        if d == b'\x00':
            return b''
        if len(d) < 2:
            raise TransformError("Transform 13 reverse: data too short")
        repeat_byte = d[0]
    ...
    def reverse_transform_14(self, d):
        if not d: raise TransformError("Transform 14 reverse: data too short")
        return d[:-1]
    ...
    def reverse_transform_15(self, d):
        if d == b'\x00':
            return b''
        if len(d) < 2:
            raise TransformError("Transform 15 reverse: data too short")
        pattern_index = d[0]
        t = bytearray(d[1:])
        pattern_values = self._get_pattern(3, pattern_index)
        for i in range(0, len(t), 3):
    ...
    def transform_23(self, data: bytes) -> bytes:
        ...
        for i, tok in enumerate(tokens):
            if i % 2 == 1:
                ...
                idx = word_to_idx.get(word_bytes)
                if idx is None:
                    idx = len(word_list)
                    word_to_idx[word_bytes] = idx
                    word_list.append(word_bytes)
                token_stream.append((1, idx))
            else:
                literal_bytes = tok.encode('utf-8')
        ...
        for wb in word_list:
            ...
            out += wb
        for typ, payload in token_stream:
            if typ == 1:
                out += b'\x01'
    ...
    def reverse_transform_23(self, data: bytes) -> bytes:
        ...
        if flag == 1:
            ...
            for _ in range(num_words):
                if pos + 4 > len(data):
                    raise TransformError("Transform 23 reverse: truncated word length")
                wlen = struct.unpack('>I', data[pos:pos+4])[0]
                pos += 4
                if pos + wlen > len(data):
                    raise TransformError("Transform 23 reverse: truncated word")
            ...
            while pos < len(data):
                ...
                if typ == 1:
                    if pos + 4 > len(data):
                        raise TransformError("Transform 23 reverse: truncated index")
                    idx = struct.unpack('>I', data[pos:pos+4])[0]
                    pos += 4
                    if idx >= len(word_list):
                        raise TransformError("Transform 23 reverse: index out of range")
                    out += word_list[idx]
                elif typ == 0:
                    if pos + 4 > len(data):
                        raise TransformError("Transform 23 reverse: truncated literal length")
                    lit_len = struct.unpack('>I', data[pos:pos+4])[0]
                    pos += 4
                    if pos + lit_len > len(data):
                        raise TransformError("Transform 23 reverse: truncated literal")
    ...
    def _dynamic_dict_detokenize(self, data: bytes) -> Optional[bytes]:
        ...
        index_bytes = data[0]
        if index_bytes not in (2, 3, 8):
            raise TransformError(f"Dynamic dict: invalid index_bytes {index_bytes}")
        pos = 1
        if pos + 4 > len(data):
            raise TransformError("Dynamic dict: truncated num_entries")
        num_entries = struct.unpack('>I', data[pos:pos+4])[0]
        pos += 4
        dictionary = []
        for _ in range(num_entries):
            if pos + 4 > len(data):
                raise TransformError("Dynamic dict: truncated chunk length")
            chunk_len = struct.unpack('>I', data[pos:pos+4])[0]
            pos += 4
            if pos + chunk_len > len(data):
                raise TransformError("Dynamic dict: truncated chunk data")
        ...
        while pos < len(data):
            ...
            if idx >= len(dictionary):
                raise TransformError(f"Dynamic dict: index {idx} out of range (max {len(dictionary)-1})")
            tokens.append(dictionary[idx])
        try:
            text = ''.join(tokens)
            return text.encode('utf-8')
        except Exception as e:
            raise TransformError(f"Dynamic dict: failed to reassemble text: {e}")
    ...
    def reverse_transform_27(self, data: bytes) -> bytes:
        if len(data) < 1:
            raise TransformError("Transform 27 reverse: data too short")
        flag = data[0]
        if flag == 0:
            return data[1:]
        if flag != 1:
            raise TransformError(f"Transform 27 reverse: unknown flag {flag}")
        payload = data[1:]
        if len(payload) < 4:
            raise TransformError("Transform 27 reverse: payload too short for length header")
        num_chars = struct.unpack('<I', payload[:4])[0]
        packed = payload[4:]
        needed_bytes = (num_chars * 6 + 7) // 8
        if len(packed) != needed_bytes:
            raise TransformError(f"Transform 27 reverse: length mismatch (expected {needed_bytes}, got {len(packed)})")
        pad_bits = (8 - (num_chars * 6) % 8) % 8
        if pad_bits > 0 and packed:
            last_byte = packed[-1]
            mask = (1 << pad_bits) - 1
            if (last_byte & mask) != 0:
                raise TransformError("Transform 27 reverse: invalid padding bits")
        bits = []
        for b in packed:
            for i in range(7, -1, -1):
                bits.append((b >> i) & 1)
        ...
        if len(bits) < needed_bits:
            raise TransformError("Transform 27 reverse: insufficient bits")
        chars = []
        for i in range(num_chars):
            ...
            if val >= 64:
                raise TransformError(f"Transform 27 reverse: invalid 6-bit value {val}")
            chars.append(SIXBIT_TO_CHAR[val])
        try:
            return ''.join(chars).encode('utf-8')
        except Exception as e:
            raise TransformError(f"Transform 27 reverse: encoding failed: {e}")
    ...
    def reverse_transform_28(self, data: bytes) -> bytes:
        if data == b'\x00':
            return b''
        if not data:
            raise TransformError("Transform 28 reverse: data is empty")
        pad_len = data[0]
        payload = data[1:]
        if len(payload) % 3 != 0:
            raise TransformError("Transform 28 reverse: payload length not multiple of 3")
        out = bytearray()
    ...
    def reverse_transform_29(self, data: bytes) -> bytes:
        ...
        if not data or len(data) < 3:
            raise TransformError("Transform 29 reverse: data too short")
        pad_len = data[0]
        if len(data) < 1 + 2:
            raise TransformError("Transform 29 reverse: missing key")
        key = int.from_bytes(data[1:3], 'little')
        payload = data[3:]
        if len(payload) % 3 != 0:
            raise TransformError("Transform 29 reverse: payload length not multiple of 3")
        out = bytearray()
        for i in range(0, len(payload), 3):
            chunk = payload[i:i+3]
            val = int.from_bytes(chunk, 'little')
    ...
    def reverse_transform_30(self, data: bytes) -> bytes:
        ...
        if not data or len(data) < 4:
            raise TransformError("Transform 30 reverse: data too short")
        pad_len = data[0]
        if len(data) < 1 + 3:
            raise TransformError("Transform 30 reverse: missing key")
        key = int.from_bytes(data[1:4], 'little')
        payload = data[4:]
        if len(payload) % 3 != 0:
            raise TransformError("Transform 30 reverse: payload length not multiple of 3")
        out = bytearray()
        for i in range(0, len(payload), 3):
            chunk = payload[i:i+3]
            val = int.from_bytes(chunk, 'little')
    ...
    def _paqjp_transform_23(self, data: bytes) -> bytes:  # index 33
        if not data: return b'\x00\x00\x00\x00\x00'
        bits = []
        for byte in data:
            for i in range(7, -1, -1):
            ...
            while pos < nbits:
                ...
                for length in range(2, 10):
                    ...
                    if key in _CONST_DIAPASON_ITER_DECODE:
                        ...
                        break
                if not matched:
                    raise TransformError("Constant Diapason reverse: invalid codeword")
            new_bits = []
    ...
    def _paqjp_transform_24(self, data: bytes) -> bytes:   # index 34
        if not data: return struct.pack('>I', 0)
        MAX_LEN = 43
        bits = []
        i = 0
    ...
    def _paqjp_reverse_24(self, data: bytes) -> bytes:
        ...
        while pos < nbits and len(out) < orig_len:
            ...
            if flag == 1:
                if pos + 8 + 6 > nbits:
                    raise TransformError("Block run reverse: truncated run block")
                ...
                if pos + 6 > nbits:
                    raise TransformError("Block run reverse: truncated literal block header")
    ...
    def _paqjp_reverse_26(self, data: bytes) -> bytes:
        if data == b'\x01\x00':
            return b''
        if not data or len(data) < 2:
            raise TransformError("FLT 26 reverse: data too short")
        n = data[0] | (data[1] << 8)
        if n % 2 == 0:
            n ^= 1
        e = pow(n, 16777216, 256) | 1
    ...
    def _paqjp_transform_27(self, data: bytes) -> bytes:   # index 37
        if not data:
    ...
    def _paqjp_reverse_27(self, data: bytes) -> bytes:
        ...
        if len(payload) % block_total_len != 0:
            raise TransformError("FLT 27 reverse: payload not aligned to blocks")
        num_blocks = len(payload) // block_total_len
        decoded = bytearray()
        for block_idx in range(num_blocks):
            offset = block_idx * block_total_len
            if offset + 2 > len(payload):
                raise TransformError("FLT 27 reverse: truncated block header")
            n = payload[offset] | (payload[offset+1] << 8)
            chunk = payload[offset+2:offset+2+BLOCK_SIZE]
            if len(chunk) < BLOCK_SIZE:
                raise TransformError("FLT 27 reverse: truncated block data")
            n |= 1
            e = pow(n, 16777216, 256) | 1
            e200 = pow(e, 200, 256)
    ...
    def _compress_backend_with_flag(self, data: bytes) -> bytes:
        ...
        if HAS_ZSTD:
            try:
                candidates.append((1, zstd_cctx.compress(data)))
            except:
                pass
        if paq is not None:
            try:
                candidates.append((2, paq.compress(data)))
            except:
                pass
        candidates.append((0, data))
        best_flag, best_data = min(candidates, key=lambda x: len(x[1]))
        return bytes([best_flag]) + best_data

    def _decompress_backend_with_flag(self, data: bytes) -> bytes:
        if not data:
            return b''
        flag = data[0]
        payload = data[1:]
        ...
        elif flag == 2 and paq is not None:
            return paq.decompress(payload)
        else:
            raise TransformError("Backend flag invalid or backend not available")

    def _paqjp_transform_28(self, data: bytes) -> bytes:   # index 38
        BLOCK_SIZE = 1024
        if not data:
            block = b'\x00' * BLOCK_SIZE
            n = 1
            transformed = block
            compressed = self._compress_backend_with_flag(transformed)
            out = bytearray(struct.pack('>I', 0))
            out += bytes([n & 0xFF, (n >> 8) & 0xFF])
            out += bytes([(len(compressed) >> 8) & 0xFF, len(compressed) & 0xFF])
            out += compressed
            return bytes(out)
        total_blocks = (len(data) + BLOCK_SIZE - 1) // BLOCK_SIZE
        out = bytearray()
        ...
        for block_idx in range(total_blocks):
            ...
            for i in range(BLOCK_SIZE):
                transformed[i] = (pow(transformed[i] + 1, e200, 257) - 1) & 0xFF
            compressed = self._compress_backend_with_flag(bytes(transformed))
            out.append(n & 0xFF)
            out.append((n >> 8) & 0xFF)
    ...
    def _paqjp_reverse_28(self, data: bytes) -> bytes:
        if not data or len(data) < 4:
            raise TransformError("FLT 28 reverse: data too short")
        orig_len = int.from_bytes(data[:4], 'big')
        payload = data[4:]
        pos = 0
        decoded = bytearray()
        while pos < len(payload):
            if pos + 2 > len(payload):
                raise TransformError("FLT 28 reverse: truncated block header")
            n = payload[pos] | (payload[pos+1] << 8)
            pos += 2
            if pos + 2 > len(payload):
                raise TransformError("FLT 28 reverse: truncated block length")
            comp_len = (payload[pos] << 8) | payload[pos+1]
            pos += 2
            if pos + comp_len > len(payload):
                raise TransformError("FLT 28 reverse: truncated compressed block")
            comp_block = payload[pos:pos+comp_len]
            pos += comp_len
            block = self._decompress_backend_with_flag(comp_block)
            n |= 1
            e = pow(n, 16777216, 256) | 1
            e200 = pow(e, 200, 256)
            inv_e200 = pow(e200, -1, 256)
    ...
    def _paqjp_transform_29(self, data: bytes) -> bytes:   # index 39
        BLOCK_SIZE = 32
        if not data:
            block = b'\x00' * BLOCK_SIZE
            n = 1
    ...
    def _paqjp_reverse_29(self, data: bytes) -> bytes:
        if not data or len(data) < 4:
            raise TransformError("FLT 29 reverse: data too short")
        orig_len = int.from_bytes(data[:4], 'big')
        payload = data[4:]
        pos = 0
        decoded = bytearray()
        while pos < len(payload):
            if pos + 2 > len(payload):
                raise TransformError("FLT 29 reverse: truncated block header")
            n = payload[pos] | (payload[pos+1] << 8)
            pos += 2
            if pos + 2 > len(payload):
                raise TransformError("FLT 29 reverse: truncated block length")
            comp_len = (payload[pos] << 8) | payload[pos+1]
            pos += 2
            if pos + comp_len > len(payload):
                raise TransformError("FLT 29 reverse: truncated compressed block")
            comp_block = payload[pos:pos+comp_len]
            pos += comp_len
            block = self._decompress_backend_with_flag(comp_block)
    ...
    def _paqjp_transform_30(self, data: bytes) -> bytes:   # index 40
        BLOCK_SIZE = 33
        if not data:
            block = b'\x00' * BLOCK_SIZE
            n, enc_n = self._paqjp_compute_n_for_block(block, 0, 0)
            compressed = self._compress_backend_with_flag(block)
            out = bytearray(struct.pack('>I', 0))
            out += enc_n
            out.append((len(compressed) >> 8) & 0xFF)
            out.append(len(compressed) & 0xFF)
        ...
        for block_idx in range(total_blocks):
            ...
            compressed = self._compress_backend_with_flag(chunk)
            out.extend(enc_n)
            out.append((len(compressed) >> 8) & 0xFF)
            out.append(len(compressed) & 0xFF)
            out.extend(compressed)
    ...
    def _paqjp_reverse_30(self, data: bytes) -> bytes:
        if not data or len(data) < 4:
            raise TransformError("FLT 30 reverse: data too short")
        orig_len = int.from_bytes(data[:4], 'big')
        payload = data[4:]
        pos = 0
        decoded = bytearray()
        while pos < len(payload):
            if pos >= len(payload):
                raise TransformError("FLT 30 reverse: truncated n length")
            Ln = payload[pos]
            pos += 1
            if Ln > 32 or pos + Ln > len(payload):
                raise TransformError("FLT 30 reverse: invalid n length")
            n_bytes = payload[pos:pos+Ln]
            pos += Ln
            if pos + 2 > len(payload):
                raise TransformError("FLT 30 reverse: truncated block length")
            comp_len = (payload[pos] << 8) | payload[pos+1]
            pos += 2
            if pos + comp_len > len(payload):
                raise TransformError("FLT 30 reverse: truncated compressed block")
            comp_block = payload[pos:pos+comp_len]
            pos += comp_len
            block = self._decompress_backend_with_flag(comp_block)
            decoded.extend(block)
    ...
    def reverse_transform_45(self, data: bytes) -> bytes:
        ...
        if original_len == 0:
            return b''
        if max(code_lengths) > 32:
            raise TransformError("Huffman reverse: code length exceeds 32")
        code_to_sym = {}
        ...
        nbits = len(bits)
        out = bytearray()
        max_code_len = max(code_lengths) if any(code_lengths) else 0
    ...
    # ----- NEW Quantum N-bit XOR transforms -----
    def _transform_nbit_quantum(self, data: bytes, key_bits: int) -> bytes:
        """Generic N-bit key transform to maximise zero bits."""
        key_bytes = key_bits // 8
        if not data:
            return b'\x00' + b'\x00' * key_bytes
        pad_len = (key_bytes - len(data) % key_bytes) % key_bytes
        padded = data + b'\x00' * pad_len
        n_blocks = len(padded) // key_bytes
        ones = [0] * key_bits
        for i in range(n_blocks):
            block = padded[i*key_bytes:(i+1)*key_bytes]
            val = int.from_bytes(block, 'big')
            for bit in range(key_bits):
                if (val >> (key_bits - 1 - bit)) & 1:
                    ones[bit] += 1
        key = 0
        for bit in range(key_bits):
            if ones[bit] * 2 > n_blocks:
                key |= (1 << (key_bits - 1 - bit))
        out = bytearray()
        for i in range(n_blocks):
            block = padded[i*key_bytes:(i+1)*key_bytes]
            val = int.from_bytes(block, 'big') ^ key
            out.extend(val.to_bytes(key_bytes, 'big'))
        return bytes([pad_len]) + key.to_bytes(key_bytes, 'big') + bytes(out)

    def _reverse_transform_nbit_quantum(self, data: bytes, key_bits: int) -> bytes:
        key_bytes = key_bits // 8
        if len(data) < 1 + key_bytes:
            raise TransformError(f"{key_bits}-bit quantum reverse: data too short")
        pad_len = data[0]
        key = int.from_bytes(data[1:1+key_bytes], 'big')
        payload = data[1+key_bytes:]
        if len(payload) % key_bytes != 0:
            raise TransformError(f"{key_bits}-bit quantum reverse: payload not aligned to {key_bytes} bytes")
        out = bytearray()
        for i in range(0, len(payload), key_bytes):
            block = payload[i:i+key_bytes]
            val = int.from_bytes(block, 'big') ^ key
            out.extend(val.to_bytes(key_bytes, 'big'))
        if pad_len > 0:
            out = out[:-pad_len]
    ...
    def transform_64bit_quantum(self, data: bytes) -> bytes:
        return self._transform_nbit_quantum(data, 64)
    def reverse_transform_64bit_quantum(self, data: bytes) -> bytes:
        return self._reverse_transform_nbit_quantum(data, 64)

    def transform_128bit_quantum(self, data: bytes) -> bytes:
        return self._transform_nbit_quantum(data, 128)
    def reverse_transform_128bit_quantum(self, data: bytes) -> bytes:
        return self._reverse_transform_nbit_quantum(data, 128)

    def transform_256bit_quantum(self, data: bytes) -> bytes:
        return self._transform_nbit_quantum(data, 256)
    def reverse_transform_256bit_quantum(self, data: bytes) -> bytes:
        return self._reverse_transform_nbit_quantum(data, 256)

    def transform_1024bit_quantum(self, data: bytes) -> bytes:
        return self._transform_nbit_quantum(data, 1024)
    def reverse_transform_1024bit_quantum(self, data: bytes) -> bytes:
        return self._reverse_transform_nbit_quantum(data, 1024)

    def transform_2048bit_quantum(self, data: bytes) -> bytes:
        return self._transform_nbit_quantum(data, 2048)
    def reverse_transform_2048bit_quantum(self, data: bytes) -> bytes:
        return self._reverse_transform_nbit_quantum(data, 2048)

    # ----- Dynamic transforms 48-56, 63-255 -----
    ...
    def reverse_transform_57(self, data: bytes) -> bytes:
        if len(data) < 5:
            raise TransformError("Transform 57 reverse: data too short")
        pad_len = data[0]
        key = int.from_bytes(data[1:5], 'little')
        payload = data[5:]
        if len(payload) % 4 != 0:
            raise TransformError("Transform 57 reverse: payload not multiple of 4")
        transformed = bytearray()
    ...
    def _build_transform_maps(self):
        ...
        # 58 = 64-bit quantum XOR
        self.fwd_transforms[58] = self.transform_64bit_quantum
        self.rev_transforms[58] = self.reverse_transform_64bit_quantum

        # 59 = 128-bit quantum XOR
        self.fwd_transforms[59] = self.transform_128bit_quantum
        self.rev_transforms[59] = self.reverse_transform_128bit_quantum

        # 60 = 256-bit quantum XOR
        self.fwd_transforms[60] = self.transform_256bit_quantum
        self.rev_transforms[60] = self.reverse_transform_256bit_quantum

        # 61 = 1024-bit quantum XOR
        self.fwd_transforms[61] = self.transform_1024bit_quantum
        self.rev_transforms[61] = self.reverse_transform_1024bit_quantum

        # 62 = 2048-bit quantum XOR
        self.fwd_transforms[62] = self.transform_2048bit_quantum
        self.rev_transforms[62] = self.reverse_transform_2048bit_quantum
    ...
    # ----- Quantum permutation transforms -----
    def _generate_permutation_from_circuit(self, num_qubits: int, seed: int, perm_size: Optional[int] = None) -> List[int]:
        """
        Generate a pseudo-random permutation of size perm_size using either a quantum circuit
        (for small qubit counts) or a classical RNG seeded by the qubit count.
        """
        if perm_size is None:
            perm_size = 1 << num_qubits  # fallback, but not used for large q

        # For large qubit counts, we avoid constructing actual quantum circuits
        if num_qubits > 12:
            rng = random.Random(seed + num_qubits * 1000000)
    ...
    def _precompute_quantum_transforms(self):
        ...
        # For q > 12, we use a block size of 1024 to keep memory bounded
        if q <= 12:
            block_size = 1 << q
        else:
            block_size = 1024
        ...
        for i in range(num_perms):
            seed = 1000 + i
            perm = self._generate_permutation_from_circuit(q, seed, perm_size=block_size)
    ...
    def _verify_lossless(self, original: bytes, transformed: bytes, rev_func: Callable) -> bool:
        try:
            restored = rev_func(transformed)
    ...
    def _decode_lzh(self, data: bytes) -> Optional[bytes]:
        ...
        if len(data) < LIT_LEN_BYTES + DIST_LEN_BYTES + LEN_LEN_BYTES:
            raise TransformError("LZH decode: data too short for code tables")
        pos = 0
        ...
        payload = data[pos:]
        if len(payload) < 4:
            raise TransformError("LZH decode: payload too short for token count")
        ...
        for _ in range(token_count):
            if bpos >= len(bits):
                raise TransformError("LZH decode: unexpected end of bitstream")
            ...
            if flag == 0:
                ...
                for cl in range(1, max_lit_bits + 1):
                    ...
                    if (cl, val) in lit_decode:
                        ...
                        break
                if not found:
                    raise TransformError("LZH decode: literal symbol not found")
            else:
                found_d = False
                for cl in range(1, max_dist_bits + 1):
                    ...
                    if (cl, val) in dist_decode:
                        ...
                        break
                if not found_d:
                    raise TransformError("LZH decode: distance symbol not found")
                found_l = False
                for cl in range(1, max_len_bits + 1):
                    ...
                    if (cl, val) in len_decode:
                        ...
                        break
                if not found_l:
                    raise TransformError("LZH decode: length symbol not found")
    ...
    def compress_quantum_nbit(self, data: bytes, key_bits: int) -> bytes:
        raw_header = self._encode_marker_raw()
        best = raw_header + self._compress_backend(data)
        transform_map = {64: 58, 128: 59, 256: 60, 1024: 61, 2048: 62}
        t_idx = transform_map.get(key_bits)
        if t_idx is None:
            raise ValueError("Unsupported key_bits")
        try:
            transformed = self.fwd_transforms[t_idx](data)
            if self._verify_lossless(data, transformed, self.rev_transforms[t_idx]):
                candidate = self._encode_marker_single(t_idx) + self._compress_backend(transformed)
                if len(candidate) < len(best):
                    best = candidate
        except Exception:
            pass
        decomp, _ = self._decompress_auto(best)
        if decomp != data:
    ...
    def compress_quantum_64(self, data: bytes) -> bytes:
        return self.compress_quantum_nbit(data, 64)

    def compress_file_quantum_nbit(self, infile: str, key_bits: int = 64, outfile: str = ""):
        try:
            ...
            return
        try:
            compressed = self.compress_quantum_nbit(data, key_bits)
        except RuntimeError as e:
            print(f"Compression failed: {e}")
            return
        if not outfile:
            outfile = self._auto_output_name(infile, ".pjp")
        try:
            self._atomic_write(outfile, compressed)
        except Exception as e:
            print(f"Error writing output: {e}")
            return
        print(f"Quantum {key_bits}-bit key compressed {len(data)} → {len(compressed)} bytes → {outfile}")

    def compress_file_quantum_64(self, infile: str, outfile: str = ""):
        self.compress_file_quantum_nbit(infile, 64, outfile)
        ...
        while time.time() - start_time < time_limit:
            ...
            try:
                ...
                for idx in seq_indices:
                    t1, t2 = self.pair_lookup[idx]
                    transformed = self.fwd_transforms[t1](transformed)
                    if not self._verify_lossless(data, transformed,
                        lambda d, idx=idx, orig=data: self._reverse_deep_sequence(d, seq_indices[:seq_indices.index(idx)+1])):
                        all_ok = False
                        break
                    transformed = self.fwd_transforms[t2](transformed)
    ...
    def compress_file_ultra_plus(self, infile: str, outfile: str = "", time_limit: float = 300.0):
        ...
        try:
            ...
            print(f"Error writing output: {e}"); return
    ...
    def _decompress_auto(self, data: bytes) -> Tuple[bytes, Optional[Tuple[int, ...]]]:
        if data[0] == 251:
            ...
            if res is None:
                raise DecompressionError("Invalid multi-pair header")
            offset, pairs = res
            payload = data[offset:]
            decompressed = self._decompress_backend(payload)
            if decompressed is None:
                raise DecompressionError("Backend decompression failed for multi-pair")
            result = decompressed
        ...
        offset, seq = self._decode_header(data)
        if offset == 0:
            raise DecompressionError("Invalid compression header")
        payload = data[offset:]
        if not payload:
            raise DecompressionError("Empty payload")
        res = self._decompress_backend(payload)
        if res is None:
            raise DecompressionError("Backend decompression failed")
        if not seq:
            return res, None
    ...
    def full_self_test(self) -> bool:
        ...
        print("=" * 60)
        print("Note: This test may take several minutes on slower systems.")
        test_byte_values = [0x00, 0xFF, 0xAA, 0x55]
        all_ok = True
        for test_byte in test_byte_values:
            test_data = bytes([test_byte])
            print(f"\nTesting byte value {test_byte:#04x}...")
            for index in range(65536):
                try:
                    ...
                    if restored != test_data:
                        print(f"  FAIL: byte {test_byte:#04x}, index {index}, seq {self.get_transform_sequence(index)}")
                        all_ok = False
                        break
                except TransformError:
                    continue
                except Exception as e:
                    print(f"  EXCEPTION at byte {test_byte:#04x}, index {index}: {e}")
                    all_ok = False
                    break
            ...
            if not all_ok:
                break
        if all_ok:
            print("\n  All sequence indices processed on test bytes 0x00, 0xFF, 0xAA, 0x55.")
            print("  Mismatches would have caused failure; TransformErrors are allowed to be skipped.")
            print("  NOTE: this is a sanity check only, not a proof of universal losslessness.")
            print("  Actual compression verifies every candidate transform on the real input;")
            print("  unsafe transforms are rejected, and raw fallback guarantees lossless output.")
        else:
            print("\n  Self‑test FAILED – some sequence is not lossless for a test byte.")
            return False
...
def main():
    ...
    n_transforms = 256
    pair_variations = n_transforms * n_transforms - 1
    two_pairs = pair_variations ** 2
    three_pairs = pair_variations ** 3

    print("=" * 58)
    print("VARIATION COUNTS (based on 256 base transforms):")
    print(f"  Single transforms          = {n_transforms}")
    print(f"  Single pairs  (256² - 1)   = {pair_variations}")
    print(f"  Two pairs     (65535²)     = {two_pairs}  (~4.3 billion)")
    print(f"  Three pairs   (65535³)     = {three_pairs}  (~281 trillion)")
    print("=" * 58)
    print("\nOptions:")
    print("  1) Fast       – 256 single transforms")
    print("  2) Ultra      – 65,535 single pairs (exhaustive)")
    print("  3) Ultra LZH  – same pairs + LZ77+Huffman")
    print("  4) Ultra++    – exhaustive 65,536 pairs + deep multi‑pair (max 300s)")
    print("  7) Deep Ultra – random search over 1..3 pairs (up to 281 trillion)")
    print("  9) Quantum N-bit Key Search (64/128/256/1024/2048)")

    while True:
        print("\nMenu:")
        print("1) Compress (Fast) – only 256 single transforms")
        print("2) Compress (Ultra) – all 65,535 pairs + backend (max 300s)")
        print("3) Compress (Ultra LZH) – pairs + LZ77+Huffman (max 300s)")
        print("4) Compress (Ultra++) – 65,536 pairs + deep multi‑pair search (max 300s)")
        print("5) Decompress")
        print("6) Full self‑test (all 65,535 indices)")
        if USE_QUANTUM and HAS_QISKIT:
            print("7) Configure quantum qubits (current: {})".format(c.QUANTUM_QUBITS))
            print("   (Variation count = 2^{} = {})".format(c.QUANTUM_QUBITS, c.get_quantum_variation_count()))
        print("8) Compress (Deep Ultra) – multi‑pair sequences (heuristic, up to 3 pairs)")
        print("9) Compress (Quantum N-bit Key Search)")
        print("0) Exit")
        choice = input("> ").strip()
        if choice == "1":
            ...
            try:
                q = int(input("Enter new qubit count (1-2048): ").strip())
            ...
            infile = input("Input file: ").strip()
            kb = input("Key bits (64, 128, 256, 1024, 2048) [default 64]: ").strip()
            try: kb = int(kb) if kb else 64
            except: kb = 64
            if kb not in [64, 128, 256, 1024, 2048]:
                print("Invalid key bits. Using 64.")
                kb = 64
            c.compress_file_quantum_nbit(infile, kb)
        elif choice == "0":
            break
        else:
